"""Transformer-based diffusion model for floorplan generation.

Ported from ``external/house_diffusion/house_diffusion/transformer.py`` with
identical architecture.  Only import paths, style, and logging were adjusted.
"""
...
def dec2bin(xinp: th.Tensor, bits: int) -> th.Tensor:
    """Convert integer tensor to binary representation.

    Args:
        xinp: Integer tensor to convert.
        bits: Number of output bits per element.

    Returns:
        Float tensor with an extra trailing dimension of size *bits*.
    """
...
def attention(
    ...
    Args:
        ...
        mask: Optional attention mask (1 = masked position).
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        Args:
            q: Query tensor of shape ``[B, S, D]``.
            k: Key tensor of shape ``[B, S, D]``.
            v: Value tensor of shape ``[B, S, D]``.
...
class EncoderLayer(nn.Module):
    """Transformer encoder layer with three parallel attention branches.

    Uses separate multi-head attention for door, self, and general masks.
    """
    ...
    def forward(
        ...
        Args:
            ...
            door_mask: Attention mask for door connections.
            self_mask: Attention mask for self connections.
            gen_mask: General attention mask (values must be 0 or 1).
...
class TransformerModel(nn.Module):
    """The full Transformer model with timestep embedding for floorplan diffusion.

    This is a direct port of the HouseDiffusion Transformer architecture,
    preserving the same layer count (4), hidden dim, and attention pattern.
    """
    ...
    def __init__(
        ...
        if use_unet:
            ...
            # preserved for interface compatibility but should not be activated
            # (use_unet must be False).
            raise NotImplementedError(
                "UNet branch is not implemented. Set use_unet=False."
            )
    ...
    def expand_points(self, points: th.Tensor, connections: th.Tensor) -> th.Tensor:
        """Expand edge points by interpolating between connected pairs.

        Args:
            points: Tensor of shape ``[B, N, C]`` with point coordinates.
            connections: Tensor of shape ``[B, N, 2]`` with connection indices.

        Returns:
            Expanded points tensor with 9x the coordinate columns.
        """

        def average_points(point1: th.Tensor, point2: th.Tensor) -> th.Tensor:
    ...
    def create_image(
        self,
        points: th.Tensor,
        connections: th.Tensor,
        room_indices: th.Tensor,
        img_size: int = 256,
        res: int = 200,
    ) -> th.Tensor:
        """Rasterize edge segments into an image tensor.

        Args:
            points: Edge point coordinates of shape ``[B, N, C]``.
            connections: Connection indices of shape ``[B, N, 2]``.
            room_indices: Room index per point of shape ``[B, N, 1]``.
            img_size: Output image resolution (square).
            res: Number of interpolation samples along each segment.

        Returns:
            Image tensor of shape ``[B, 1, img_size, img_size]``.
        """
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        Args:
            x: An ``[N x C x S]`` Tensor of inputs.
            timesteps: A 1-D batch of timesteps.
            xtalpha: Signal scaling factor for binary branch.
