...
Ported from ``external/house_diffusion/house_diffusion/gaussian_diffusion.py`` with
identical math.  Only import paths and device handling were adjusted.
"""
...
def get_named_beta_schedule(
    ...
    if schedule_name == "linear":
        ...
        logger.debug("Using cosine beta schedule")
...
def betas_for_alpha_bar(
    ...
    The function defines the cumulative product of (1-beta) over time from t = [0,1].
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        Args:
            ...
            analog_bit: Controls binary/analog prediction branching.

        Returns:
            A dict with keys 'mean', 'variance', 'log_variance', 'pred_xstart'.
        """
        ...
        b, c = x.shape[:2]
        assert t.shape == (b,)
        xtalpha = _extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x.shape).permute(
            [0, 2, 1]
        )
        epsalpha = _extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, x.shape).permute(
            [0, 2, 1]
        )
    ...
    def _predict_xstart_from_xprev(
        self, x_t: th.Tensor, t: th.Tensor, xprev: th.Tensor
    ) -> th.Tensor:
    ...
    def _predict_eps_from_xstart(
        self, x_t: th.Tensor, t: th.Tensor, pred_xstart: th.Tensor
    ) -> th.Tensor:
        return (
            _extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x_t.shape) * x_t
            - pred_xstart
    ...
    def condition_mean(
        self, cond_fn, p_mean_var: dict, x: th.Tensor, t: th.Tensor, model_kwargs=None
    ) -> th.Tensor:
    ...
    def p_sample(
        ...
        Returns:
            A dict containing 'sample' and 'pred_xstart'.
        """
    ...
    def _vb_terms_bpd(
        ...
        Returns:
            A dict with 'output' (shape [N] tensor of NLLs or KLs) and 'pred_xstart'.
        """
    ...
    def training_losses(
        ...
        Args:
            ...
            model_kwargs: If not None, a dict of extra keyword arguments to
                pass to the model. This can be used for conditioning.
            analog_bit: Whether to use analog bit representation.
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.model_var_type in [
                ...
                b, c = x_t.shape[:2]
                assert model_output.shape == (b, c * 2, *x_t.shape[2:])  # noqa: F821
                model_output, model_var_values = th.split(model_output, c, dim=1)  # noqa: F821
            ...
            if not analog_bit:
                ...
                bin_target = bin_target.reshape([target.shape[0], target.shape[2], 16]).permute(
                    [0, 2, 1]
                )
                t_weights = (t < 10).to(x_start.device).unsqueeze(1).unsqueeze(2)
    ...
    def calc_bpd_loop(
        ...
        Returns:
            A dict with keys 'total_bpd', 'prior_bpd', 'vb', 'xstart_mse', 'mse'.
        """
